...
export async function POST(request: NextRequest) {
  ...
  if (authError) {
    console.error("API setup-db: supabase.auth.getUser() error", authError);
  ...
  if (!user || user.email !== ADMIN) {
    console.warn(`API setup-db: unauthorized attempt by ${user?.email}`);
    return NextResponse.json(
  ...
  if (!databaseUrl) {
    console.error("API setup-db: DATABASE_URL env var is not set.");
    return NextResponse.json(
  ...
  try {
    ...
    console.log("API setup-db: Advisory lock acquired.");

    console.log("API setup-db: Skipping direct modification of 'auth.users'.");
    ...
    `);
    console.log("API setup-db: Table 'workspaces' ensured.");
    ...
    `);
    console.log("API setup-db: Sharing columns ensured on 'workspaces'.");
    ...
    console.log("API setup-db: RLS enabled for 'workspaces'.");
    ...
    `);
    console.log("API setup-db: RLS policies set for 'workspaces'.");
    ...
    `);
    console.log("API setup-db: Table 'stages' ensured.");

    await client.query(`
      ALTER TABLE public.stages
      ADD COLUMN IF NOT EXISTS workspace_id uuid NOT NULL REFERENCES public.workspaces(id) ON DELETE CASCADE;
    `);
    console.log("API setup-db: Column 'workspace_id' ensured on 'stages'.");
    ...
    `);
    console.log("API setup-db: RLS policy set for 'stages'.");
    ...
    `);
    console.log("API setup-db: Table 'projects' ensured.");
    ...
    `);
    console.log("API setup-db: Columns ensured on 'projects'.");
    ...
    await client.query(`
      ...
      WITH CHECK (auth.uid() = user_id);
    `);
    console.log("API setup-db: RLS policy set for 'projects'.");
    ...
    `);
    console.log("API setup-db: Table 'vers' ensured.");
    ...
    `);
    console.log("API setup-db: Columns ensured on 'vers'.");

    // Add index on vers.project_id
    await client.query(`
      CREATE INDEX IF NOT EXISTS idx_vers_project_id ON public.vers(project_id);
    `);
    console.log("API setup-db: Index 'idx_vers_project_id' ensured.");
    ...
    await client.query(`
      ...
      WITH CHECK (auth.uid() = user_id);
    `);
    console.log("API setup-db: RLS policy set for 'vers'.");
    ...
    `);
    console.log("API setup-db: Table 'tasks' ensured.");
    ...
    `);
    console.log("API setup-db: Columns ensured on 'tasks'.");
    ...
    `);
    console.log("API setup-db: RLS policy set for 'tasks'.");
    ...
    `);
    console.log("API setup-db: Table 'tags' ensured.");

    await client.query(`
      ALTER TABLE public.tags
      ADD COLUMN IF NOT EXISTS is_global BOOLEAN NOT NULL DEFAULT FALSE;
    `);
    console.log("API setup-db: Column 'is_global' ensured on 'tags'.");
    ...
    console.log("API setup-db: Predefined global tags inserted or updated.");
    ...
    `);
    console.log("API setup-db: Table 'task_tags' ensured.");
    ...
    `);
    console.log("API setup-db: Table 'shared_tag_access' ensured.");
    ...
    console.log("API setup-db: RLS enabled for 'tags'.");
    ...
    await client.query(`
      ...
      USING (auth.uid() = user_id);
    `);
    console.log("API setup-db: RLS policies set for 'tags'.");
    ...
    `);
    console.log("API setup-db: RLS policies set for 'task_tags'.");
    ...
    console.log("API setup-db: Permissions granted to 'authenticated' role.");
    ...
    await client.query(`
      ...
      $$ LANGUAGE plpgsql;
    `);
    console.log("API setup-db: RPC function 'create_stage_at_start' ensured.");

    // Grant execute permission on the function
    await client.query(`GRANT EXECUTE ON FUNCTION public.create_stage_at_start(UUID, TEXT) TO authenticated;`);
    console.log("API setup-db: EXECUTE permission granted on 'create_stage_at_start'.");

    // Commit everything
    await client.query("COMMIT");
    console.log("API setup-db: Multi-user database setup successful.");
    return NextResponse.json({ success: true, message: "Multi-user database setup successful." });
  } catch (err) {
    // Rollback on error
    await client.query("ROLLBACK");
    console.error("API setup-db: Error during multi-user DB setup:", err);
    ...
    client.release();
    await pool.end();
    console.log("API setup-db: Client released, pool ended.");
  }
}
